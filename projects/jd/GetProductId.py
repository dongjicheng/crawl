...
class GetProductId(SpiderManger):
    def __init__(self, seeds_file, **kwargs):
        ...
        with op.DBManger() as m:
            ...
            total = m.count(db_collect=("jingdong", last_brand_collect))
            pipeline = [
                {"$match":
                     {"brand_id": {"$ne": "null"}}
                 }
    ...
    def make_request(self, seed):
        if seed.type == 0:
            cate_id, _, name = re.split(',', seed.value)
            cate_id, name = urllib.parse.urlencode({"cat": cate_id}), urllib.parse.urlencode({"ev": "exbrand_" + name})
            cid1, cid2, cid3 = re.split(',', cate_id)
            url = 'https://list.jd.com/list.html?{0}&{1}&cid3={3}'.format(cate_id, name, cid3)
            request = {"url": url,
                       "method:":"get",
                       "encoding":"utf-8",
                       "proxies": {"http": random.choice(self.proxies)},
                       "headers": {"Connection": "close", "User-Agent": self.ua.chrome,
                                   "Referer": "https://list.jd.com/list.html?{0}&cid3={1}&cid2={2}".format(cate_id, cid3, cid2)}}
            ...
            cate_id, _, name, page, s = re.split(',', seed.value)
            cate_id, name = urllib.parse.urlencode({"cat": cate_id}), urllib.parse.urlencode({"ev": "exbrand_" + name})
            url = 'https://list.jd.com/list.html?{0}&{1}&page={2}&s={3}&click=1'.format(cate_id, name, page, s)
            request = {"url": url,
                       "method:":"get",
                       "encoding":"utf-8",
                       "proxies": {"http": random.choice(self.proxies)},
            ...
            if r1:
                ...
                if r1:
                    self.seeds_queue.put(
                        Seed(value=(seed.value[0], seed.value[1], seed.value[2], page+1, s+30, r1), retries=self.retries,
                             type=1))
                    self.progress_decrease()
                    buffer = []
                    for pid in r1.split(","):
                        buffer.append({"pid":pid,"cate_id":cate_id,"_seed":seed})
                    self.write(buffer)
        elif seed.type == 2:
            cate_id, _, name, page, s, items = re.split(',', seed.value)
            cate_id, name = urllib.parse.urlencode({"cat": cate_id}), urllib.parse.urlencode({"ev": "exbrand_" + name})
            url = 'https://list.jd.com/list.html?{0}&{1}&page={2}&s={3}&scrolling=y&log_id=1596108547754.6591&tpl=1_M&isList=1&show_items={4}'.format(cate_id, name, page,s,items)
            request = {"url": url,
                       "method:":"get",
                       "encoding":"utf-8",
                       "proxies": {"http": random.choice(self.proxies)},
                       "headers": {"Connection": "close", "User-Agent": self.ua.chrome,
                                   "Referer": "https://list.jd.com/list.html?{0}&{1}&page={2}&s={3}&click=1".format(cate_id, name, page-1,s-30)}}
            rs = self.get_request(request)
            r1 = self.first_pettern.findall(rs)
            if r1:
                r1 = r1[0]
